...
| ✍️ **텍스트 레이어** | 진짜 폰트로 글자를 얹는다 — **오타 없음·수정 즉시·무료**. 한글 폰트 4종(고딕·포스터 제목·명조·손글씨), 색·크기·테두리. 완성되면 이미지로 굽기 |
| ✂️ **오려내기** | 원본 픽셀 그대로 두고 배경만 투명 — **로컬 모델(BiRefNet, MIT)·API 비용 0원**. 사진·로고처럼 원본이 변하면 안 될 때 |
| 🫥 **투명 배경 PNG** | ① 처음부터 배경 없이 생성(토글) ② AI가 피사체를 다시 그려 배경 제거 — 일러스트용 (gpt-image-1) |
