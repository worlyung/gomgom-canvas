...
| 🎯 **캠페인 한 번에** | "○○ 축제 캠페인 만들어줘" 한 줄 → 키비주얼·포스터·배너·굿즈로 **자동 분해**, 계획 확인 후 일괄 생성. 첫 키비주얼을 참조로 물려 **톤이 통일** |
...
| 📋 **협업 보드** | 프로젝트를 서버에 저장해 **여러 보드를 오가고**, 다른 사람이 이어받는다. 동시 저장은 충돌 경고로 덮어쓰기 방지 |
...
**🎯 캠페인 한 번에** — "부천 벚꽃 야행 축제 캠페인" 한 줄로 4종을 자동 분해·생성 (키비주얼 톤이 나머지에 그대로 이어짐)

![캠페인 자동 분해 결과 4종](docs/images/campaign.jpg)
...
- 협업 보드는 **순차 협업**(열기→작업→저장)이다. 실시간 동시 편집은 지원하지 않으며, 같은 보드를 동시에 저장하면 나중 저장자에게 충돌 경고가 뜬다
- 보드는 서버의 `boards/` 폴더에 저장된다(저장소에는 올라가지 않음)
