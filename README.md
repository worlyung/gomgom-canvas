...
| 🫥 **투명 배경 PNG** | ① 처음부터 배경 없이 생성(토글) ② **이미 만든 이미지의 배경 제거** — 둘 다 (gpt-image-1 자동 전환) |
...
**🫥 배경 제거** — 이미 만든 이미지에서 피사체만 남긴 투명 PNG (오른쪽 바둑판이 비치는 부분이 투명)

![배경 제거 전후 비교](docs/images/remove-bg.jpg)
